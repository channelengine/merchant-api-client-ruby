  ...
  class MerchantReturnRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ChannelEngineMerchantApiClient::MerchantReturnRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ChannelEngineMerchantApiClient::MerchantReturnRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @merchant_return_no.to_s.length > 50
        invalid_properties.push('invalid value for "merchant_return_no", the character length must be smaller than or equal to 50.')
      end

      if @merchant_return_no.to_s.length < 0
        invalid_properties.push('invalid value for "merchant_return_no", the character length must be great than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if @merchant_return_no.to_s.length > 50
      return false if @merchant_return_no.to_s.length < 0
    ...
    # Custom attribute writer method with validation
    # @param [Object] merchant_return_no Value to be assigned
    def merchant_return_no=(merchant_return_no)
      if merchant_return_no.nil?
        fail ArgumentError, 'merchant_return_no cannot be nil'
      end

      if merchant_return_no.to_s.length > 50
        fail ArgumentError, 'invalid value for "merchant_return_no", the character length must be smaller than or equal to 50.'
      end

      if merchant_return_no.to_s.length < 0
        fail ArgumentError, 'invalid value for "merchant_return_no", the character length must be great than or equal to 0.'
      end

      @merchant_return_no = merchant_return_no
    end
    ...
    def hash
      [merchant_order_no, merchant_return_no, lines, id, reason, customer_comment, merchant_comment, refund_incl_vat, refund_excl_vat, return_date, extra_data].hash
    end
